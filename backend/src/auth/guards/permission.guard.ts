...
export class PermissionsGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    const permissionConfig = this.reflector.getAllAndOverride<any>(
      PERMISSIONS_KEY,
  ...
  private checkAllPermissions(
    ...
    const hasPermissions = this.permissionsService.hasAllPermissions(
      user.role,
      permissions,
    );
  ...
  private checkOwnershipPermission(
    ...
    const hasPermission = this.permissionsService.hasContextualPermission(
      context,
      permission,
    );
